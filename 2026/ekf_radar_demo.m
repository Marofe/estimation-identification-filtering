...
x = zeros(5,N);
x(:,1) = [ 70; 60; 7.0; deg2rad(150); deg2rad(1.5) ];   % [px py v phi Om]
for k = 1:N-1
    % scripted manoeuvre: change the commanded turn-rate a few times
    if     k==20, x(5,k) = deg2rad(-3.5);
    elseif k==40, x(5,k) = deg2rad( 4.0);
    elseif k==55, x(5,k) = deg2rad(-2.0);
    end
...
lims = [min([x(1,:) mx])-25, max([x(1,:) mx])+25, min([x(2,:) my])-25, max([x(2,:) my])+25];
...
for k = 1:N
    ...
    draw_hexacopter(ax, x(1:2,k), x(4,k), 4.2, cK, cB);
...
function draw_radar(ax, c)
    % small antenna/dish glyph at the origin
    plot(ax,0,0,'^','MarkerSize',12,'MarkerFaceColor',c,'MarkerEdgeColor',c);
    plot(ax,[0 0],[0 6],'-','Color',c,'LineWidth',1.5);
    th=linspace(-pi/3,pi/3,20);
    plot(ax, 5*sin(th), 6+5*(cos(th)-1)*0.5, '-','Color',c,'LineWidth',1.5);
    text(ax, 3, -6, 'RADAR','Color',c,'FontSize',9,'FontWeight','bold');
end
