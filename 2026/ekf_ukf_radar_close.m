...
%% ---- parameters ----
T=1.0; N=45;
sig_r=1.5; sig_th=deg2rad(1.0); R=diag([sig_r^2, sig_th^2]);
q_v=0.25^2; q_Om=deg2rad(1.2)^2; Q=diag([1e-4,1e-4,q_v,1e-6,q_Om]);

%% ---- ground truth (compact curvy path) ----
x=zeros(5,N); x(:,1)=[-150;26;7.5;deg2rad(-5);deg2rad(0.6)];   % flies across, passing close to the radar
for k=1:N-1
    if k==28, x(5,k)=deg2rad(2.4); end
