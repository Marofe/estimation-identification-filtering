...
for fidx=1:numel(sth_deg)
    ...
    % means (MC circle, Taylor square, UT star)
    hmMC =plot(ax2,muMC(1),muMC(2),'o','MarkerFaceColor',cMC ,'MarkerEdgeColor','k','MarkerSize',9,'LineWidth',1);
    hmTAY=plot(ax2,muT(1) ,muT(2) ,'s','MarkerFaceColor',cTAY,'MarkerEdgeColor','k','MarkerSize',11,'LineWidth',1);
    hmUT =plot(ax2,muU(1) ,muU(2) ,'p','MarkerFaceColor',cUT ,'MarkerEdgeColor','k','MarkerSize',16,'LineWidth',1);
    xlabel(ax2,'p_x  [m]'); ylabel(ax2,'p_y  [m]');
    xlim(ax2,[-7 10]); ylim(ax2,[1 12]);
    legend(ax2,[hMC hTAY hUT hmMC hmTAY hmUT], ...
        {'MC ellipse (true)','Taylor / EKF ellipse','UT ellipse', ...
         'MC mean','Taylor mean','UT mean'}, ...
        'Location','southwest','FontSize',9,'Box','off','NumColumns',2);
